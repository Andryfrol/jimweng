...
var (
	...
	hostIp   = "https://10.2.10.119:9440/api/nutanix/v2.0/cluster/"
)
...
func main() {
	...
	jsonResp := new(AutoGenerated)
	bodyBytes, _ := ioutil.ReadAll(resp.Body)
	json.Unmarshal(bodyBytes, &jsonResp)

	// for i, v := range jsonResp.RackableUnits {
	// 	fmt.Printf("%v : %v\n", i, v)
	// }
	defer resp.Body.Close()

	fmt.Printf("jsonResp %v\n", jsonResp.Stats.IoBandwidthKBps)
	i, _ := strconv.Atoi(jsonResp.Stats.IoBandwidthKBps)
	fmt.Printf("%v\n", i)
...
type AutoGenerated struct {
	...
	ClusterRedundancyState struct {
		...
		} `json:"redundancy_status"`
	} `json:"cluster_redundancy_state"`
	...
	} `json:"security_compliance_config"`
	HypervisorSecurityComplianceConfig struct {
		...
		EnableHighStrengthPassword bool   `json:"enable_high_strength_password"`
		EnableBanner               bool   `json:"enable_banner"`
	} `json:"hypervisor_security_compliance_config"`
	IscsiConfig struct {
		ExternalClientEnabled bool `json:"external_client_enabled"`
	} `json:"iscsi_config"`
	...
	UsageStats struct {
		...
		StorageTierDasSataPinnedBytes                string `json:"storage_tier.das-sata.pinned_bytes"`
	...
	EnableOnDiskDedup                 interface{} `json:"enable_on_disk_dedup"`
}
